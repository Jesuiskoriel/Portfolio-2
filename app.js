    ...
    const translations = {
      ...
      en: {
        ...
        journeyTitle: "What Sets Me Apart",
        journeyText: "4 key points that define how I work.",
    ...
    function Nav({ t, lang, setLang, theme, setTheme }) {
      ...
      const [isPillMoving, setIsPillMoving] = useState(false);
      const linksRef = useRef(null);
      const linkRefs = useRef({});
      const moveTimeoutRef = useRef(null);
      const navItems = useMemo(() => [
        { href: "#accueil", id: "accueil", label: t.nav[0] },
        { href: "#projets", id: "projets", label: t.nav[1] },
        { href: "#services", id: "services", label: t.nav[2] },
        { href: "#competences", id: "competences", label: t.nav[3] },
        { href: "#veille", id: "veille", label: t.nav[4] },
        { href: "#contact", id: "contact", label: t.nav[5] }
      ], [t]);

      const placePill = (href) => {
        ...
        const left = Math.max(0, Math.round(linkRect.left - linksRect.left));
      ...
      useEffect(() => {
        ...
        const onScroll = () => {
          ...
          navItems.forEach((item) => {
            ...
            if (top <= marker) current = item.href;
          });
      ...
      useEffect(() => {
        placePill(hoverHref || activeHref);
      }, [hoverHref, activeHref, lang]);

      useEffect(() => {
        const onResize = () => placePill(hoverHref || activeHref);
        window.addEventListener("resize", onResize);
        return () => window.removeEventListener("resize", onResize);
      }, [hoverHref, activeHref, lang]);

      useEffect(() => {
        setIsPillMoving(true);
        if (moveTimeoutRef.current) window.clearTimeout(moveTimeoutRef.current);
        moveTimeoutRef.current = window.setTimeout(() => {
          setIsPillMoving(false);
          moveTimeoutRef.current = null;
        }, 360);
      }, [hoverHref, activeHref]);
      ...
      return (
        <nav className="nav liquid-nav">
          ...
          <ul
            className={`links${hoverHref ? " is-hovering" : ""}${isPillMoving ? " is-moving" : ""}`}
            ref={linksRef}
            onMouseLeave={() => setHoverHref(null)}
            style={{
              "--nav-pill-left": `${pill.left}px`,
              "--nav-pill-width": `${pill.width}px`,
              "--nav-pill-opacity": pill.visible ? 1 : 0,
              "--nav-pill-scale": hoverHref ? 1.035 : 1,
              "--nav-pill-sheen-opacity": hoverHref ? 0.92 : 0.72
            }}
          >
            {navItems.map((item) => (
              <li key={item.href}>
                <a
                  ...
                  ref={(node) => {
                    if (node) linkRefs.current[item.href] = node;
                    else delete linkRefs.current[item.href];
                  }}
      ...
      return (
        <>
          ...
          <div className="flow-stack">
            ...
            <div className="flow-item" style={{ "--i": 2 }}><Goal t={t} /></div>
            <div className="flow-item" style={{ "--i": 3 }}><Projects t={t} lang={lang} /></div>
            <div className="flow-item" style={{ "--i": 4 }}><Services t={t} /></div>
            <div className="flow-item" style={{ "--i": 5 }}><Skills t={t} /></div>
            <div className="flow-item" style={{ "--i": 6 }}><FavoriteTech t={t} /></div>
            <div className="flow-item" style={{ "--i": 7 }}><Journey t={t} /></div>
            ...
            <div className="flow-item" style={{ "--i": 12 }}><Footer t={t} /></div>
          </div>
        </>
      );
    }
